...
file_name = 'ba295b7e-7893-4ae3-a2f7-54c5f0223e45_uofa'
data_file = pd.read_csv(path+file_name+'.csv')
...
for i in range(len(data_file.iloc[:,0])):
    try:
        ...
        X_i = pd.read_csv(io.StringIO(content.decode('utf-8')))
        name_file = str(data_file.iloc[i,1].split("/")[4])
        for chanel in range(num_chanel):
            img_mat = (np.array(X_i.iloc[:,2:12])[:,chanel]).reshape(350,350)[100:-100,100:-100]
            img = PIL.Image.fromarray(np.uint8(img_mat))
